...
crp <- function(data, position, answer, id,
  ...
  #We need the 0 frequency lags too
  all_lags = sort(c((1:(nrow(key) - 1)) * -1, 1:(nrow(key) - 1))) #Get the full range of lags
  
  temp_id = c()
  temp_all = c()
  
  for (s in unique(DF$Sub.ID)){
    
    temp_id = c(temp_id, rep(s, times = length(all_lags)))
    temp_all = c(temp_all, all_lags)
                   
  }
  
  all_lags2 = data.frame(temp_id, temp_all) 
  
  colnames(all_lags2)[1:2] = c("Sub.ID", "participant_lags") #Gets a dataframe of sub.IDs and all the full range of lags. Will use this for merging at the end
  ...
  for (i in DF$Sub.ID){
    ...
    if (nrow(temp_part) > 1){
      ...
      ##do it again for the all_lags df
      ...
      for (col in other.columns){
        DF_temp <- unique(DF[ , c("Sub.ID", col)])
        if (sum(duplicated(DF_temp$Sub.ID)) == 0){
          all_lags3 <- merge(all_lags2, DF_temp, by = "Sub.ID")
        }
      }
  ...
  DF_final_TEMP = merge(all_lags3, DF_final, by = c("Sub.ID", "participant_lags"), all = T)
  DF_final_TEMP$participant_lags = as.numeric(DF_final_TEMP$participant_lags)
  DF_final_TEMP = DF_final_TEMP[ , -6]
  colnames(DF_final_TEMP)[3] = gsub(names(DF_final_TEMP[3]), pattern = ".x", replacement = "")
  
  DF_final_TEMP$Freq[is.na(DF_final_TEMP$Freq)] = 0
  DF_final_TEMP$Possible.Freq[is.na(DF_final_TEMP$Possible.Freq)] = 0
  DF_final_TEMP$CRP[is.na(DF_final_TEMP$CRP)] = 0
